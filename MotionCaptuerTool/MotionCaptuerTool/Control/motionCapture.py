...
chFileDir = os.path.join(appPath, 'file')
chFile = os.path.join(chFileDir, 'MC.ma')
namespace= 'mcData'

def makePreviw(path, size):
    ...
    parentCon = pm.pointConstraint(hipJnt,camNode, mo=False)
    pm.delete(parentCon)
    
    pm.lookThru( camNode )
    pm.select('bodyMesh', r=True)
    ...
    pm.playblast(fp=4, clearCache=1, showOrnaments=0, sequenceTime=0, format='image', percent=100, 
                 filename=path , viewer=0, quality=100, widthHeight=(size, size), compression="png")
    
    pm.lookThru( 'persp' )
    pm.delete(camNode, tempFloor, pointCon)
    
def importCharactor(*argv):
...
def makeAsset( source_mcDir=None, 
    ...
    for i, sourceFile in enumerate(fileList):
        ...
        # 애니메이션 클립 내보냄 
        # 프로젝트의 clip폴더로 내보냄.
        if not os.path.exists(export_clipDir):
            os.makedirs(export_clipDir)
        ...
        makePreviw(os.path.join(previewDir,withoutExtFileName), size)
        
        # 파일저장
        exportPath = os.path.join( export_mcDir, sceneName)
        if not os.path.exists(export_mcDir):
            os.makedirs(export_mcDir)
        if os.path.exists(exportPath):
            shutil.rmtree(exportPath)
...
def importFile(filePath, *argv):
    pm.importFile(filePath)

def openFile(filePath, *argv):
    pm.openFile(filePath)

def referenceFile(filePath, *argv):
    pm.createReference(filePath, namespace="Temp")
